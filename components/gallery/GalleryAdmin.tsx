...
import { getGalleryItems, addGalleryItem, deleteGalleryItem } from '@/lib/firebase/gallery'
...
export function GalleryAdmin() {
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    try {
      const newItem = await addGalleryItem(file, label.trim(), category, items.length)
    ...
    <div className="flex flex-col gap-4">
      ...
      {adding && (
        <div className="bg-white rounded-3xl shadow-sm border-[1.5px] border-sky-200 p-5 flex flex-col gap-4">
          ...
          <label
            className="border-2 border-dashed border-[#e5e5e5] rounded-2xl bg-[#f5f5f7] h-32 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-sky-300 hover:bg-sky-50 transition-colors overflow-hidden"
          >
            {preview ? (
