  ...
  return (
    <main className="min-h-screen bg-[#f5f5f7]">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-[#f0f0f0] sticky top-0 z-20">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          {user && (
            <button
              type="button"
              onClick={() => { signOut(auth); setName(''); setNameConfirmed(false) }}
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {/* Step 2 — logged in, needs to enter name */}
        {user && !nameConfirmed && (
          <>
            <div className="text-center mb-6">
              <h1 className="text-2xl font-bold text-[#1d1d1f]">קביעת תור</h1>
              <p className="text-sm text-[#6e6e73] mt-1">שלב אחרון לפני בחירת הזמן</p>
            </div>
        ...
        {/* Step 3 — logged in + name confirmed → show calendar */}
        {user && nameConfirmed && (
          <>
            <div className="flex items-center justify-between mb-5">
              <div>
                <h1 className="text-2xl font-bold text-[#1d1d1f]">שלום, {name} 👋</h1>
                <p className="text-sm text-[#6e6e73] mt-0.5">בחרי תאריך ושעה שנוחים לך</p>
              </div>
              <button
                type="button"
